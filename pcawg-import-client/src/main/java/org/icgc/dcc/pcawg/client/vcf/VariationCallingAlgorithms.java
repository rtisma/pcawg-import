...
public enum VariationCallingAlgorithms {
  ...
  public static String get(WorkflowTypes workflowType, DataTypes dataType){
    checkArgument(workflowMap.containsKey(workflowType), "The workflowType [%s] is not defined in %s",workflowType, VariationCallingAlgorithms.class.getName());
    val dataTypeMap = workflowMap.get(workflowType);
    checkArgument(dataTypeMap.containsKey(dataType), "The dataType [%s] is not defined for workflowType [%s] in %s", dataType, workflowType, VariationCallingAlgorithms.class.getName());
